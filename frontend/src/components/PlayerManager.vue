<template>
  <div class="player-manager bg-white rounded-lg shadow-md p-6">
      ...
      <form @submit.prevent="addPlayer" class="space-y-2">
        <div class="flex gap-2">
          ...
          <button
            type="submit"
            :disabled="loading"
            class="bg-green-600 text-white py-2 px-4 rounded-md hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
          >
            Add Player
          </button>
        </div>
        <p class="text-xs text-gray-500">Leave rating blank for unrated players</p>
      </form>
    </div>
    ...
    <ul v-else class="space-y-2">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <span v-if="player.rating" class="text-sm text-gray-600">
            ({{ player.rating }})
          </span>
          <span v-else class="text-xs text-gray-400 italic">(unrated)</span>
        </div>
        <button
